<template>
  <div class="blog-comment-form-wrapper my-10">
    <div class="blog-comment-form-title">
      <h2 class="title">Để lại bình luận:</h2>
    </div>
    <div class="comment-box">
      <form action="" @submit.prevent="postComment(productDetail.id)">
        <div class="row">
...
export default {
  ...
  methods: {
    async postComment(id) {
      if (this.userInformation !== null) {
        ...
        const res = await ProductService.comment(this.data);
        if (res) {
          this.comment = "";
        }
      } else {
  ...
  mounted() {
    this.data = this.productDetail.comment;
  },
};
</script>
